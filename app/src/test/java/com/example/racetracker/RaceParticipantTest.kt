...
//        Fungsi advanceTimeBy() membantu mengurangi waktu eksekusi uji.

        runCurrent()
//    Fungsi ini akan menjalankan semua tugas yang tertunda pada saat ini.

        assertEquals(expectedProgress, raceParticipant.currentProgress)
    }

    @Test
    fun raceParticipant_RaceFinish_ProgressUpdated() = runTest{
        launch {
            raceParticipant.run()
        }
...
//    fungsi assertEquals() untuk memastikan nilai properti raceParticipant.currentProgress sama dengan 100.
    }
    ...
    fun raceParticipant_ProgressIcrementZero_ExceptionThrown() = runTest{
